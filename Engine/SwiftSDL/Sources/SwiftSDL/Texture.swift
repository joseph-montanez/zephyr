@dynamicMemberLookup
public protocol Texture: AnyObject, SDLObjectProtocol, Sendable where Pointer == UnsafeMutablePointer<SDL_Texture> { }
...
public func SDL_CreateTexture<P: PropertyValue>(with properties: [(String, value: P)], renderer: any Renderer) throws(SDL_Error) -> some Texture {
    ...
    return SDLObject(pointer, tag: "texture", destroy: SDL_DestroyTexture)
}
...
extension Texture {
    ...
    public var renderer: Result<any Renderer, SDL_Error> {
        self
            ...
            .map({ SDLObject($0, tag: "texture.renderer") })
    }
}
...
extension Renderer {
    ...
    public func texture(from surface: any Surface, transparent: Bool = false, tag: String? = nil) throws(SDL_Error) -> any Texture {
        ...
        return SDLObject(pointer, tag: tag ?? "texture (from surface)", destroy: SDL_DestroyTexture)
    }
    ...
    public func texture(from bitmap: inout [UInt8], transparent: Bool = false, tag: String? = nil) throws(SDL_Error) -> any Texture {
        ...
        let surface: any Surface = SDLObject(pointer, tag: tag ?? "surface (bitmap)", destroy: SDL_DestroySurface)
